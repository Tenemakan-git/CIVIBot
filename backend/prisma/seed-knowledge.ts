...
/**
 * Enrichissement du corpus RAG (base de connaissances vectorielle).
 *
 * Contrairement à `seed.ts` — qui insérait les chunks avec un embedding NUL
 * (vecteur zéro) « pour la simulation » — ce script génère de VRAIS embeddings
 * via Voyage AI. Sans cela, la similarité cosinus est dégénérée et le
 * Knowledge Agent ne franchit jamais le seuil → tout bascule en recherche web
 * (statut « needs_followup »/`!`).
 *
 * Idempotent : pour chaque document, on upsert l'entête puis on RÉ-INDEXE
 * intégralement ses chunks (delete + insert) avec des embeddings frais.
 * Rejouable à volonté (`npm run seed:knowledge`).
 */

// ── Fournisseur d'embeddings (aligné sur VoyageAdapter : voyage-3, 1024-dim) ──
...
const CORPUS: DocSpec[] = [
  // ===== Existant : ré-indexé avec de vrais embeddings =====
  {
    id: 'doc-sarl-ohada',
  ...
  // ===== Création d'entreprise : nouvelles couvertures =====
  {
  ...
  // ===== État civil & identité : nouvelles couvertures =====
  {
...
async function main() {
  ...
  // 1) Embeddings de tous les chunks (batchés pour limiter les appels API).
  const flat: { docId: string; ordre: number; page: number; texte: string }[] = [];
  for (const doc of CORPUS) {
    doc.chunks.forEach((texte, i) =>
      flat.push({ docId: doc.id, ordre: i + 1, page: i + 1, texte }),
    );
  }
  console.log(
    `🧠 Génération des embeddings Voyage (${flat.length} chunks, modèle ${VOYAGE_MODEL})…`,
  );
  ...
  // 2) Upsert documents + ré-indexation des chunks (idempotent).
  for (const doc of CORPUS) {
    ...
    // Purge des anciens chunks (notamment ceux à embedding nul du seed initial).
    await prisma.knowledgeChunk.deleteMany({ where: { documentId: doc.id } });
  }

  // 3) Insertion des chunks avec embeddings réels.
