...
exports.templatesForDomaine = templatesForDomaine;
exports.DOCUMENT_TEMPLATES = [
    {
        key: 'demande-acte-naissance',
        titre: "Demande de copie d'acte de naissance",
        description: "Demande manuscrite type adressée à l'officier de l'état civil pour obtenir une ou plusieurs copies de votre acte de naissance.",
        domaines: ['etat_civil'],
    ...
    },
    {
        ...
        description: "Lettre de demande d'immatriculation adressée au Guichet Unique du CEPICI pour la création de votre entreprise.",
    ...
    },
    {
        ...
        description: "Lettre formelle générique pour solliciter un document ou une démarche auprès d'une administration. Le corps est rédigé automatiquement selon votre objet.",
...
function getTemplate(key) {
    return exports.DOCUMENT_TEMPLATES.find((t) => t.key === key);
}
function templatesForDomaine(domaine, procedureSlug) {
    return exports.DOCUMENT_TEMPLATES.filter((t) => {
        if (!t.domaines.includes(domaine))
            return false;
        if (!t.procedureSlugs || t.procedureSlugs.length === 0)
            return true;
