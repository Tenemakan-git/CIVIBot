/**
 * Questionnaire branché du parcours guidé (Côte d'Ivoire : état civil +
 * création d'entreprise). Arbre de questions : chaque option mène à la question
 * suivante (`next`) ou termine le parcours (`next` absent). Piloté côté backend
 * pour rester maintenable (et éditable par un admin en phase 2).
 */

export interface QuestionnaireOption {
  value: string;
  label: string;
  /** Id de la question suivante ; absent = fin du parcours. */
  next?: string;
}
